...
export class MailService {
    constructor(
      ...
      // api로 post request보내기... private 상태에선 테스트 못함
    async sendEmail(subject:string, template:string, emailVars:EmailVar[]) : Promise<boolean> { // 프론트에서 fetch 같이 api를 이용하기 위해 got설치
      // Form data는 node 에서 스트림을 만드는 라이브러리           // ↑이메일변수의 배열          
        ...
        try{
            await got.post(`https://api.mailgun.net/v3/${this.options.domain}/messages`,{ // URL + domain + messages
